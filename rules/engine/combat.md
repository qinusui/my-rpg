...
**初始化**：grep bestiary.md → 查 world_constants.json → `combat.py --init <monster_key> [--count N] [--threat skirmish|battle|boss|ambush]`（自动触发 `bg.py --combat`）→ AskUserQuestion（header="战斗"）
...
**结束**：`state_mgr.py --clear_encounter` → `state_mgr.py --set current_location <id>`（自动触发 `bg.py --set`）
