...
def _determine_modules(state: Dict[str, Any], environment_result: Dict[str, Any]) -> List[str]:
    modules: List[str] = ["rules/engine/main_loop.md"]

    player_name = state.get("player_name", "")
    if player_name in ("冒险者", "无名者", ""):
        modules.append("rules/engine/character_creation.md")

    if state.get("pending_encounter") or environment_result.get("encounter"):
        modules.append("rules/engine/combat.md")

    if state.get("current_goal"):
        modules.append("rules/engine/goals.md")

    health = state.get("health", {})
    spirit = state.get("spirit", {})
    if isinstance(health, dict) and health.get("current", 0) >= health.get("max", 10):
        modules.append("rules/engine/endings.md")
