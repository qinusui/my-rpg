...
**ANSI 颜色分层** — 用内联 ANSI 区分信息类型，建立视觉层次：

```
叙事正文     \033[0m  白色（默认）
系统信息     \033[90m 暗灰 — 骰子结果、属性变化、命令输出
DM 提示      \033[90m 暗灰 — 旁白、规则提示
重要警告     \033[31m 红色 — 濒死、满钟触发、不可逆后果
大成功       \033[32m 绿色 — D20=20
大失败       \033[31m 红色 — D20=1
```

全局前景色已移除——颜色层完全由 ANSI 内联控制，DM 按需使用。
