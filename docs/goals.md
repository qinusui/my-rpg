...
DM 从 `character_options.json` 中取出目标选项，用 AskUserQuestion 展示。玩家选定目标后，额外执行一步：

**誓言仪式** — 玩家用自己的话说出誓言的措辞。DM 将原话写入目标：

```
python tools/state_mgr.py --set_oath "我以初代王的名义发誓，找到全部八片王冠碎片，终结龙神的阴影"
```
